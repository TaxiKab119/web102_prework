...
const totalRaised = GAMES_JSON.reduce( (acc, game) => {
    return acc + parseInt(game['pledged']);
}, 0);
...
const numUnfunded = GAMES_JSON.reduce( (sum, game) => {
    // Add one to sum only if pledged is less than goal
    const add = (game['pledged'] < game['goal']) ? 1 : 0;
    return sum + add;    
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
const gameFundingStr = `A total of $${totalRaised.toLocaleString('en-US')} 
...
callToAction.innerHTML = gameFundingStr;
...
const topPledgeElement = document.createElement('p');
const secondPledgeElement = document.createElement('p');

topPledgeElement.innerHTML = topGame['name'];
secondPledgeElement.innerHTML = secondGame['name'];

document.getElementById('first-game').appendChild(topPledgeElement);
document.getElementById('second-game').appendChild(secondPledgeElement);